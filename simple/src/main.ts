import {component, field, system, System, SystemType, World} from '@lastolivegames/becsy';
import { Schema, type } from "@colyseus/schema";
import * as ColyseusJS from "colyseus.js";
...
// (created below).
@component export class Position extends Schema {
  ...
  @type("number") @field.float64 declare x: number;
  @type("number") @field.float64 declare y: number;
}

@component export class Velocity extends Schema {
  ...
  @type("number") @field.float64 declare vx: number;
  @type("number") @field.float64 declare vy: number;
}
...
@system export class VelocityInputController extends System {
  ...
  public myRoom: ColyseusJS.Room<MyRoomState>
  ...
  execute(): void {
    ...
    for (const movable of this.movables.current) {
      ...
      //FIXME: this is not allowed in server/client game => server must write velocity
...
@system export class Renderer extends System {
  ...
  inputController = this.attach(VelocityInputController);
  ...
  async execute() {
    ...
    const client = new ColyseusJS.Client(process.env.TETROLYSEUS_SERVER || 'ws://localhost:2567')
    const room = await client.joinOrCreate<MyRoomState>("tetrolyseus")
    this.inputController.myRoom = room
  }
}
